...
function(rmlui_bgfx_get_shader_variant)
    ...
    if(ARG_VARIANT STREQUAL "glsl-330")
        set(_platform linux)
        set(_profile 330)
    elseif(ARG_VARIANT STREQUAL "essl-300")
        set(_platform android)
        set(_profile 300_es)
    elseif(ARG_VARIANT STREQUAL "metal")
        set(_platform osx)
        set(_profile metal)
    else()
...
function(rmlui_bgfx_find_shader_tools)
    ...
    if(NOT _shaderc AND TARGET shaderc)
        set(_shaderc "$<TARGET_FILE:shaderc>")
    endif()
    ...
    if(ARG_REQUIRED AND NOT _shaderc)
        message(FATAL_ERROR "shaderc host executable not found. Set RMLUI_BGFX_SHADERC_EXECUTABLE or SHADERC, or enable the standalone fetched toolchain.")
    endif()
    if(ARG_REQUIRED AND _shaderc AND NOT _shaderc MATCHES "^\\$<TARGET_FILE:" AND NOT EXISTS "${_shaderc}")
        message(FATAL_ERROR "shaderc host executable does not exist: ${_shaderc}")
    endif()

    set(_bgfx_include "${RMLUI_BGFX_BGFX_SHADER_INCLUDE_DIR}")
    if(NOT _bgfx_include AND RMLUI_BGFX_FETCHED_BGFX_SOURCE_DIR)
        set(_bgfx_include "${RMLUI_BGFX_FETCHED_BGFX_SOURCE_DIR}/bgfx/src")
    endif()
...
function(rmlui_bgfx_add_shader_target)
    ...
    if(TARGET shaderc AND ARG_SHADERC STREQUAL "$<TARGET_FILE:shaderc>")
        add_dependencies(${ARG_TARGET} shaderc)
    endif()
endfunction()
